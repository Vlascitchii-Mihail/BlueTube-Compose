...
class ConverterTest {
    ...
    fun `convertToDomainYoutubeVideoResponse() converts YoutubeVideoResponseEntity to YoutubeVideoResponse`() {
        ...
        assertEquals(DOMAIN_RESPONSE_VIDEO_WITH_CHANNEL_IMG.nextPageToken, result.nextPageToken)
        assertEquals(DOMAIN_RESPONSE_VIDEO_WITH_CHANNEL_IMG.currentPageToken, result.currentPageToken)
        assertEquals(DOMAIN_RESPONSE_VIDEO_WITH_CHANNEL_IMG.prevPageToken, result.prevPageToken)

        DOMAIN_RESPONSE_VIDEO_WITH_CHANNEL_IMG.items.assertListEqualsTo(result.items)
    }

    @Test
    fun `convertToLocalYoutubeVideoResponseEntity() converts YoutubeVideoResponse to YoutubeVideoResponseEntity`() {
        val result = DOMAIN_RESPONSE_VIDEO_WITH_CHANNEL_IMG.convertToLocalYoutubeVideoResponseEntity()
